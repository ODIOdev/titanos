"use client";

import * as React from "react";
import { cn } from "@/lib/utils";
import { selectInputValueOnFocus } from "@/lib/input-focus";
import { Label } from "@/components/ui/label";

export interface MoneyInputProps
  extends Omit<
    React.InputHTMLAttributes<HTMLInputElement>,
    "type" | "value" | "onChange" | "prefix"
  > {
  label?: string;
  error?: string;
  hint?: string;
  value?: number | string | null;
  onValueChange?: (value: number | null) => void;
}

/** Format a numeric amount for display: 1234.5 → "1,234.5" */
export function formatMoneyDisplay(
  value: number | string | null | undefined,
  { fixed }: { fixed?: boolean } = {},
): string {
  if (value == null || value === "") return "";
  const amount = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(amount)) return "";
  if (fixed) {
    const [intPart, decPart = "00"] = amount.toFixed(2).split(".");
    return `${Number(intPart).toLocaleString("en-US")}.${decPart}`;
  }
  const [intPart, decPart] = String(amount).split(".");
  const withCommas = Number(intPart).toLocaleString("en-US");
  return decPart != null ? `${withCommas}.${decPart}` : withCommas;
}

/** Live-format typed money: digits + optional `.` with up to 2 decimals, commas in integer. */
export function formatMoneyTyping(raw: string): string {
  const cleaned = raw.replace(/[^\d.]/g, "");
  if (!cleaned) return "";

  const firstDot = cleaned.indexOf(".");
  const intRaw =
    firstDot === -1 ? cleaned : cleaned.slice(0, firstDot) || "0";
  const decRaw =
    firstDot === -1 ? null : cleaned.slice(firstDot + 1).replace(/\./g, "").slice(0, 2);

  const intNormalized = intRaw.replace(/^0+(?=\d)/, "") || "0";
  const withCommas = intNormalized.replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  if (firstDot !== -1) {
    return `${withCommas}.${decRaw ?? ""}`;
  }
  return withCommas;
}

export function parseMoneyInput(formatted: string): number | null {
  const trimmed = formatted.trim();
  if (!trimmed || trimmed === ".") return null;
  const n = Number(trimmed.replace(/,/g, ""));
  return Number.isFinite(n) ? n : null;
}

function isZeroMoneyText(text: string) {
  const parsed = parseMoneyInput(text);
  return parsed === 0 || text === "0" || text === "0.0" || text === "0.00";
}

const MoneyInput = React.forwardRef<HTMLInputElement, MoneyInputProps>(
  (
    {
      className,
      label,
      error,
      hint,
      id,
      value,
      onValueChange,
      onBlur,
      onFocus,
      required,
      ...props
    },
    ref,
  ) => {
    const generatedId = React.useId();
    const inputId = id ?? generatedId;
    const errorId = error ? `${inputId}-error` : undefined;
    const hintId = hint && !error ? `${inputId}-hint` : undefined;

    const [text, setText] = React.useState(() =>
      formatMoneyDisplay(value, { fixed: value != null }),
    );
    const [focused, setFocused] = React.useState(false);

    React.useLayoutEffect(() => {
      if (focused) return;
      setText(formatMoneyDisplay(value, { fixed: value != null }));
    }, [value, focused]);

    return (
      <div className="w-full">
        {label ? (
          <Label htmlFor={inputId}>
            {label}
            {required ? (
              <span className="ml-0.5 text-red-600" aria-hidden="true">
                *
              </span>
            ) : null}
          </Label>
        ) : null}
        <div className="relative">
          <span
            className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-sm text-medium-gray"
            aria-hidden="true"
          >
            $
          </span>
          <input
            {...props}
            ref={ref}
            id={inputId}
            type="text"
            inputMode="decimal"
            autoComplete="off"
            required={required}
            placeholder={props.placeholder ?? "0.00"}
            value={text}
            aria-invalid={error ? true : undefined}
            aria-describedby={errorId ?? hintId}
            className={cn(
              "flex h-10 w-full rounded-sm border border-border-gray bg-white py-2 pl-7 pr-3 text-sm text-near-black placeholder:text-medium-gray transition-colors focus-visible:border-dark-charcoal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-titan-yellow/40 disabled:cursor-not-allowed disabled:bg-light-gray disabled:opacity-60",
              error && "border-red-600 focus-visible:ring-red-200",
              className,
            )}
            onFocus={(e) => {
              setFocused(true);
              // Clear zero placeholders so the next keystroke starts fresh.
              if (isZeroMoneyText(text)) {
                setText("");
                onValueChange?.(null);
              } else {
                selectInputValueOnFocus(e);
              }
              onFocus?.(e);
            }}
            onChange={(e) => {
              const next = formatMoneyTyping(e.target.value);
              setText(next);
              onValueChange?.(parseMoneyInput(next));
            }}
            onBlur={(e) => {
              setFocused(false);
              const parsed = parseMoneyInput(text);
              if (parsed == null) {
                setText("");
                onValueChange?.(null);
              } else {
                setText(formatMoneyDisplay(parsed, { fixed: true }));
                onValueChange?.(parsed);
              }
              onBlur?.(e);
            }}
          />
        </div>
        {error ? (
          <p id={errorId} className="mt-1.5 text-sm text-red-700" role="alert">
            {error}
          </p>
        ) : hint ? (
          <p id={hintId} className="mt-1.5 text-sm text-medium-gray">
            {hint}
          </p>
        ) : null}
      </div>
    );
  },
);
MoneyInput.displayName = "MoneyInput";

export { MoneyInput };
