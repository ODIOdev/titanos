...
export function ProductCard({
  ...
  return (
    <article
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-sm border border-border-gray bg-white transition-[border-color] duration-200 hover:border-dark-charcoal",
        className,
      )}
    >
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-4">
        <div className="min-h-0 flex-1">
          <Link
            href={`/product/${product.slug}`}
            className="font-heading text-sm uppercase leading-snug tracking-wide text-dark-charcoal line-clamp-2 transition-colors hover:text-near-black sm:text-base"
          >
            {product.name}
          </Link>
          <p className="mt-1 hidden text-xs text-medium-gray group-hover:block">
            SKU: {product.sku}
          </p>
          <div className="mt-2">
            <StarRating
